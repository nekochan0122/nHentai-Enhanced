...
export function book() {
  ...
  $(
    $(
      `<h3 class="title"><span class="before">神的語言：</span><a id="book_id" class="god" data-clipboard-text="${book_id}" href="javascript:;">${book_id}</a></h3>`
    )
  ).insertAfter('#gallery_id')
  ...
  for (let i = 1, len = Object.keys(json.Book.TagsName).length, span = ''; i <= len; i++) {
    ...
    $H(
      `#tags > .tag-container:nth-child(${i})`,
      `${json.Book.TagsName[Object.keys(json.Book.TagsName).sort((a, b) => a - b)[i - 1]]} ${span}`
    )
  }
  ...
  // 新增按鈕 - 單頁閱讀
  $('#info > .buttons').prepend(
    `<a href="/g/${book_id}/1/?onePageMode=True" class="btn btn-primary"><i class="fas fa-book-open"></i> ${json.Book.Btns.Read}</a>`
  )

  // 新增按鈕 - 搜尋相關本本
  let searchText1 =
      $('#info .title').length === 2
        ? `${$('#info .title:nth-child(1) > .pretty').text()}`
        : $('#info .title').length === 3
        ? `${$('#info .title:nth-child(2) > .pretty').text()}`
        : null,
  ...
  // 搜尋相關本本 按鈕
  $('#info > .buttons').append(
    `<a id="serachRelatedBookBtn" class="btn btn-secondary" href="javascript:;" style="cursor: wait;"><i class="fas fa-search"></i> ${json.Book.Btns.Searching}</a>`
  )
    ...
    $.ajax({
      ...
      success: data => {
        ...
        function updateBtn(searchText, resultNum, j = json.Book.Btns) {
          ...
          $('#serachRelatedBookBtn')
            .css('cursor', 'pointer')
            .attr('href', `/search/?q=${searchText}`)
            .html(`<i class="fas fa-search"></i> ${btnTextresultNum} (<span>${resultNum}</span>)`)
        }
      },
      error: () => {
  ...
  if (login) {
    ...
    $H(
      '#comment-post-container > div > p',
      `<a class="login-comment" href="/login/">${json.Book.NoLogin.Login}</a> ${json.Book.NoLogin.Or} <a class="login-comment" href="/register/">${json.Book.NoLogin.Register}</a> ${json.Book.NoLogin.ToPostAComment}`
    )
  }
