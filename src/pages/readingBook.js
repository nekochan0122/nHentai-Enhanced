...
export function readingBook() {
  ...
  function ajaxPageFunc() {
    ...
    // 插入自訂版 reader-bar
    $('.reader-bar').append(
      `<div style="display:flex;align-self:flex-center;position:absolute;left:50%;transform:translateX(-50%)"><button class="page-number btn btn-unstyled"><span class="current">0</span><span class="divider">&nbsp;/&nbsp;</span><span class="num-pages">${maxNum}</span></button></div>`
    )

    // reader-bar css 及 隱藏/顯示 動畫效果
    $('.reader-bar')
      .eq(0)
      .css({ opacity: '0', position: 'fixed', top: '0', width: '100%', 'z-index': '999999' })
      .hover(
        function () {
        ...
        success: data => {
          ...
          $('#content').append(
            newHtml
              ...
              .attr('id', `page${target}`)
              .css({ display: 'block', margin: '0px auto' })
          )
